...
The plugin SHALL expose a `/codex-pro:result` skill registered at `plugins/codex-pro/skills/result/SKILL.md` with a YAML frontmatter declaring `name: result`, a descriptive `description` block whose trigger keywords include "show result file" / "display review output" / 顯示結果 / 看完整 / detail verbiage, and an `allowed-tools` list containing at least `Bash` and `Read`. The skill SHALL accept three mutually-exclusive selection modes: (a) a positional `<filename>` argument naming a file in `.codex-pro/` (filename only, no path prefix); (b) `--latest <skill>` where `<skill>` is one of `review`, `rescue`, `adversarial-review`; (c) `--latest` with no argument (selects the most recent result file across all producer skills). Supplying both a positional filename and `--latest` SHALL be rejected as a usage error.

#### Scenario: Skill is registered and discoverable

- **WHEN** the plugin is installed
- **THEN** `plugins/codex-pro/skills/result/SKILL.md` MUST exist with valid YAML frontmatter
- **AND** the frontmatter `name` field MUST equal `result`
- **AND** the frontmatter `allowed-tools` MUST contain both `Bash` and `Read`

#### Scenario: Positional filename selects a specific file

- **WHEN** a user invokes `/codex-pro:result review-20260601T120000Z.md` with that file present in `.codex-pro/`
...
- **WHEN** a user invokes `/codex-pro:result --latest rescue` with multiple `rescue-*.md` files in `.codex-pro/`
...
- **WHEN** a user invokes `/codex-pro:result --latest` with mixed-producer files in `.codex-pro/`
...
- **WHEN** a user invokes `/codex-pro:result review-20260601T120000Z.md --latest`
...
- **WHEN** the static layer inspects `plugins/codex-pro/skills/result/SKILL.md`
...
- **WHEN** a user invokes `/codex-pro:result --latest review` with `.codex-pro/` containing `review-20260601T120000Z.md` and `review-20260601T133000Z.md`
- **THEN** the skill SHALL select `review-20260601T133000Z.md` (later ISO8601 timestamp)

#### Scenario: Filesystem mtime is not consulted

- **WHEN** a user invokes `/codex-pro:result --latest` with `.codex-pro/` containing `review-20260601T120000Z.md` and `rescue-20260601T130000Z.md`, and the rescue file's filesystem `mtime` has been touched to an earlier time than the review file
...
- **WHEN** a user invokes `/codex-pro:result bogus-20260601T120000Z.md` with that file absent from `.codex-pro/`
- **THEN** the skill SHALL emit an error message referencing `/codex-pro:status` as the recovery action
- **AND** the skill SHALL exit non-zero

#### Scenario: --latest <skill> with zero matches aborts with producer remediation

- **WHEN** a user invokes `/codex-pro:result --latest adversarial-review` with `.codex-pro/` containing only `review-*.md` files
- **THEN** the skill SHALL emit an error message referencing `/codex-pro:adversarial-review` as the recovery action
- **AND** the skill SHALL exit non-zero
