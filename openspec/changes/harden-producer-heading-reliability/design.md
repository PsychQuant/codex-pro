...
**Behavior（使用者可觀察）**：`/codex-pro:review` 的輸出 result file 穩定以 `## Summary` 開頭、後接 `## Findings`，每個 finding 用 `### Finding N: <severity> — <file>:<line>` H3 heading — 與 v0.3 的契約相同，但 Codex 遵守率從「偶爾漂移」變為「e2e matrix 觀察全綠」。promotion 落地後，`bash tests/e2e.sh` 在 heading 缺失時 FAIL（exit 非 0）而非僅印 ⚠ 警告。
