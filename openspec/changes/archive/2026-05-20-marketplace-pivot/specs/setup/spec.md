...
- **AND** then installs the sub-plugin via `/plugin install codex-pro-setup@codex-pro`
- **THEN** the marketplace catalog at `.claude-plugin/marketplace.json` is parsed without error
- **AND** the `/plugin` listing shows the `codex-pro` marketplace containing the `codex-pro-setup` entry
- **AND** the `/codex-pro-setup:setup` skill becomes available

#### Scenario: Sub-plugin dev-test path loads sub-plugin directly

- **WHEN** a developer starts Claude Code with `--plugin-dir` pointing at the sub-plugin directory `plugins/codex-pro-setup` inside the codex-pro project
- **THEN** the sub-plugin manifest at `plugins/codex-pro-setup/.claude-plugin/plugin.json` is parsed without error
- **AND** the sub-plugin appears in the plugin listing under the name `codex-pro-setup`
- **AND** the `/codex-pro-setup:setup` skill becomes available without needing to add the marketplace first

#### Scenario: Sub-plugin manifest missing or malformed

- **WHEN** Claude Code attempts to load the sub-plugin and `plugins/codex-pro-setup/.claude-plugin/plugin.json` is missing or syntactically invalid
- **THEN** Claude Code SHALL report a manifest parse error and refuse to register the sub-plugin
- **AND** no `/codex-pro-setup:*` command becomes available
